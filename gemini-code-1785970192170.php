...
if ($accion === 'register') {
    ...
    if ($metodo_pago === 'stripe') {
        ...
        $datos_stripe = [
            ...
            'success_url' => "http://localhost:81/ojo%20ip/{$destino}?status=success",
            'cancel_url' => "http://localhost:81/ojo%20ip/{$destino}?status=cancel",
        ];
...
if ($accion === 'login') {
    ...
    if ($usuario_encontrado && password_verify($contrasena, $usuario_encontrado['password'])) {
        if (!$usuario_encontrado['active']) {
            echo json_encode(['success' => false, 'message' => 'Cuenta pendiente de verificación']);
            exit;
        }
